...
let init (window: IWindow) =
    ...
    let win = Silky.WebGpuWin(window, state)
    let wgpu = win.Wgpu
    ...
    win.onRender (fun view t ->
        ...
        let speed = 20f
        if Silky.keys[int Key.A] then
            posX <- posX - speed * float32 t

        if Silky.keys[int Key.D] then
            posX <- posX + speed * float32 t

        if Silky.keys[int Key.W] then
            posZ <- posZ + speed * float32 t

        if Silky.keys[int Key.S] then
            posZ <- posZ - speed * float32 t
            
        if Silky.keys[int Key.Space] then
            posY <- posY + speed * float32 t
            
        if Silky.keys[int Key.ShiftLeft] then
            posY <- posY - speed * float32 t
