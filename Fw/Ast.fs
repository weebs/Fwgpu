...
and printStmt (s: CppStmt) =
    ...
    | WhileLoop(cond, body) -> $"while ({print cond}) {{{printBody body}}}"
...
let private addReturn' (s: CppStmt) =
    ...
    | Return _ -> s
    | Assign _ -> s
    | IfThenElse(cond, wt, wf) -> IfThenElse(cond, addReturn wt, addReturn wf)
...
let rec printDecl (decl: CppDecl) =
    match decl with
    | INIT body -> $"INIT;{shadowVariables [] body |> printBody} ENDINIT;"
    | Comment s -> $"/* {s} */"
