...
let rec shortPrint depth (e: FSharpExpr) =
  ...
  | P.Call (None, mfv, [], [], []) ->
    ...
    | Some ent -> $"(Module Var?) {ent.BasicQualifiedName}.{mfv}"
    | None -> $"(Module Var?) {mfv}"
  ...
  | P.Sequential (a, b) -> $"(Sequential)"
  | P.NewObject (mfv, tys, args) -> $"NewObject {mfv}"
  | P.FSharpFieldSet (e, t, field, v) -> $"FieldSet {field.Name}"
  | P.ThisValue t -> $"ThisValue {t.BasicQualifiedName}"
  | P.Let ((mfv, e, dbgPt), body) ->
    if not mfv.IsMutable then $"Let {mfv}" else $"Let mut {mfv}"
  | P.FSharpFieldGet (e, t, field) -> $"FieldGet {field.Name}"
  | P.Lambda (mfv, body) -> $"Lambda {mfv}"
  | P.ValueSet (mfv, e) -> $"ValueSet {mfv}"
  | P.NewRecord (t, fields) -> $"New Record: {t.BasicQualifiedName}"
  | P.DefaultValue t -> $"Default Value {t.BasicQualifiedName}"
  | _ -> failwith $"Unrecognized expression %A{e}"
  |> printfn "%s"
...
let rec translate depth (e: FSharpExpr) =
  ...
  | P.Call (None, mfv, [], [], []) ->
    ...
    | None -> printfn $"Call (Var?) {mfv}"
  | P.Const (o, t) when
    t.BasicQualifiedName = "Microsoft.FSharp.Core.unit"
    ->
    printfn $"Const ()"
  | P.Const (o, t) -> printfn $"Const {o}"
  | P.Call (callee, mfv, xs, ys, args) ->
    printfn $"Call {mfv}"
  ...
  | P.FSharpFieldSet (e, t, field, v) ->
    ...
    translate (depth + 1) v
  | P.ThisValue t -> printfn $"ThisValue {t.BasicQualifiedName}"
  | P.Let ((mfv, e, dbgPt), body) ->
