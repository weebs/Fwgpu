...
module Format =
  ...
  let source (src: string) : string =
    let cmd = Cli.Wrap("clang-format").WithStandardInputPipe(PipeSource.FromString src)
...
type CppCompiler() =
  ...
  let toArgs (mfv: FsMfv) (curriedArgs: FsMfv list list) =
    ...
    curriedArgs
    |> List.collect id
    |> List.map (fun mfv -> (mfv.CompiledName, Transform.tyConvert mfv.FullType))
    |> fun args ->
        // First argument for member functions is "this", so we skip
        if mfv.IsMember then List.tail args else args
  ...
  member private this.ProcessDecl decl =
    match decl with
    | FSharpImplementationFileDeclaration.Entity(entity, declarations) -> this.ProcessEntity entity declarations
    | FSharpImplementationFileDeclaration.MemberOrFunctionOrValue(mfv, curriedArgs, body) ->
      this.ProcessMfv mfv curriedArgs body
    | FSharpImplementationFileDeclaration.InitAction action -> this.ProcessInitAction action
  ...
  member private this.Class entity declarations =
    let fields =
      entity.FSharpFields
      |> Seq.map (fun field -> (field.Name, Transform.tyConvert field.FieldType))
      |> Seq.map (fun (a, b) -> Ast.Variable(a, b, None))
      |> Seq.toList
    ...
    let ctors =
      ...
      |> Seq.map (fun args -> Ast.Constructor(entity.CompiledName, args, None))
      |> Seq.toList
